...
public class BoardDAO {
	...
	private BoardDAO() { // 임의로 객체 생성 못하도록 private로 선언
		// TODO Auto-generated constructor stub
	}

	public static BoardDAO getInstance(){ // 객체생성은 위에 생성자가 하는게 아니라 여기서 한다. 싱글톤패턴으로 사용하려고 static 붙임
		if(boardDAO == null){
	...
	public ArrayList<BoardBean> selectArticleList(int page,int limit){
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		//String board_list_sql="select * from board order by BOARD_RE_REF desc,BOARD_RE_SEQ asc limit ?,10"; // MySQL용 쿼리
		...
		int startrow=(page-1)*10; //읽기 시작할 row 번호..	(0, 10, 20,..이런식으로)
		int endrow = startrow + limit; // 읽을 마지막 row 번호 (9, 19, 29, ... )
		try{
	...
	public int insertArticle(BoardBean article){
		...
		try{
			...
			// 글번호 세팅하는 조건문
			if(rs.next())
				num =rs.getInt(1)+1; //borad테이블의 첫번째 컬럼 + 1
			else
				num=1; // 첫번째 레코드
			...
			pstmt.setInt(7, num); //어느 글의 댓글인지 파악하기 위한 숫자(원글 번호)
			pstmt.setInt(8, 0);
			pstmt.setInt(9, 0);
			pstmt.setInt(10, 0);
